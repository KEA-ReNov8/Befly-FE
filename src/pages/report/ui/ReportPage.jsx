...
export const ReportPage = () => {
  ...
  const transformReportData = (apiData) => {
    ...
    const cleanSuggestComment = resultData.suggest_comment?.replace(/\*\*/g, '') || '추가 제안이 없습니다.';
    const cleanTotalComment = resultData.total_comment?.replace(/\*\*/g, '') || '분석 내용이 없습니다.';

    return {
      title: resultData.chat_title || '제목 없음',
      analysisData: cleanTotalComment,
      scores: resultData.after_keyword?.map(item => ({
          title: item.emotion,
          value: item.score,
